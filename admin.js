...
function renderCaseDetail() {
  ...
  $('#editPersonForm').onsubmit = async e => {
    e.preventDefault(); const data = Object.fromEntries(new FormData(e.currentTarget)); data.address = `${data.street}, ${data.number}${data.apartment ? `, ${data.apartment}` : ''} · ${data.neighborhood} · ${data.city}`;
    try { const response = await api(`/api/admin/cases/${c.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); if (!response.ok) throw new Error(); await loadCases(); toast('Dados da pessoa atualizados'); } catch { toast('Não foi possível salvar as alterações'); }
  };
